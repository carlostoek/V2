...
@pytest.mark.asyncio
async def test_start_command_and_profile_callback(mocked_bot: MockedBot):
    """Verifica que el comando /start muestra el menú y el callback de perfil funciona."""
    # Arrange
    ...
    assert sent_message.text == "¡Bienvenido a Diana V2! ¿Qué te gustaría hacer hoy?"
    assert sent_message.reply_markup is not None
    assert len(sent_message.reply_markup.inline_keyboard) == 4 # 4 filas en el nuevo menú
    ...
    assert len(mocked_bot.requests) == 3
    edited_message = mocked_bot.requests[1]
    assert edited_message.text == """Este es tu perfil:

Puntos: 10"""
    assert mocked_bot.requests[2].__class__.__name__ == "AnswerCallbackQuery"