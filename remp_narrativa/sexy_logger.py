...
def log_execution_time(logger: SexyLogger, description: str = None):
...
if __name__ == "__main__":
    ...
    async def demo_sexy_logging():
        """Demo del sistema de logging sexy"""
        
        # Banner de inicio
        sexy_logger.banner(
            "🎭 BOT DIANA - SISTEMA DE VALIDACIONES",
            "Sistema de logging más sexy del mundo"
        )
        
        # Sección de inicialización
        with sexy_logger.section("INICIALIZACIÓN DE SERVICIOS", "🔧"):
            sexy_logger.startup("Cargando configuración...")
            await asyncio.sleep(0.5)
            
            sexy_logger.database("Conectando a PostgreSQL...", operation="connect")
            await asyncio.sleep(0.3)
            
            sexy_logger.startup("Inicializando Diana Validation Service...")
            await asyncio.sleep(0.2)
            
            sexy_logger.success("✅ Todos los servicios inicializados correctamente")
        
        # Sección de simulación de usuario
        with sexy_logger.section("SIMULACIÓN DE JOURNEY DE USUARIO", "🎮"):
            user_id = 12345
            
            sexy_logger.user_action("Usuario inicia el bot", user_id=user_id, action="start_bot")
            
            sexy_logger.gamification("Otorgando puntos de bienvenida", user_id=user_id, points=10)
            
            # Simular reacciones
            sexy_logger.user_action("Usuario reacciona rápidamente", user_id=user_id, action="quick_reaction")
            
            # Validación exitosa
            sexy_logger.diana_validation_success(
                user_id=user_id,
                level="level_1_to_2", 
                score=0.85,
                archetype="explorer",
            ...
            # Detectar arquetipo
            sexy_logger.diana_archetype_detected(user_id=user_id, archetype="explorer", confidence=0.92)
            
            # Entregar recompensas
            sexy_logger.diana_reward_delivered(
                user_id=user_id,
                reward_type="mochila_viajero",
                items=["pista_1", "vision_parcial"]
            )
            
            # Simular validación fallida
            sexy_logger.diana_validation_failed(
                user_id=user_id,
                level="level_2_to_3",
                score=0.45,
                reason="Insuficiente tiempo de exploración"
            )
        
        # Resumen final
        sexy_logger.summary("🏆 DEMO COMPLETADO")
    
    # Ejecutar demo
    asyncio.run(demo_sexy_logging())
